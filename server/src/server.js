...
app.get("/api/users/validate-token", async (req, res) => {
  try {
    ...
    if (error.name === "CastError") {
      console.log("MongoDB Cast error - invalid ObjectId:", error.value);
      return res.status(401).json({ message: "Invalid token format" });
    }
...
adminRoutes.get("/dashboard-stats", isAdmin, async (req, res) => {
  try {
    ...
    const [totalBooks, activeMembers, pendingApplications, membershipPlans] =
      await Promise.all([
        models.Book.countDocuments(),
        models.User.countDocuments({ hasMembership: true }),
        models.Membership.countDocuments({ status: "pending" }),
        models.MembershipPlan.countDocuments(),
      ]);
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      console.log("\n" + "=".repeat(50));
      console.log(`🚀 BookHub Server Started`);
