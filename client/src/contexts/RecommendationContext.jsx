  ...
  const fetchRecommendations = useCallback(
    async (options = {}) => {
      ...
      if (isFetchingRef.current && !force) {
        console.log(
          "Already fetching recommendations, skipping duplicate request"
        );
        return [...topPicks];
      }
      ...
      try {
        ...
        try {
          ...
          if (recommendationsArray.length > 0) {
            ...
            if (shouldUpdateState) {
              ...
              setRefreshStatus({
                isRefreshed: true,
                message: token
                  ? "Personal recommendations updated!"
          ...
          if (
            ...
            (recommendationsError.code &&
              recommendationsError.code === "ERR_CANCELED") ||
          ...
          console.warn(
            "Failed to get recommendations, trying trending instead:",
          ...
          if (trendingResponse.data) {
            if (Array.isArray(trendingResponse.data)) {
              ...
              Array.isArray(trendingResponse.data.data)
            ) {
          ...
          trendingArray = trendingArray.filter(
            (item) => item && typeof item === "object" && (item._id || item.id)
          );

          if (trendingArray.length > 0) {
            console.log(
              "Using trending books as fallback. Count:",
              trendingArray.length
            );
            ...
            if (shouldUpdateState) {
              ...
              setRefreshStatus({
                isRefreshed: true,
                message:
                  "Using trending books (couldn't get personalized recommendations)",
              });
